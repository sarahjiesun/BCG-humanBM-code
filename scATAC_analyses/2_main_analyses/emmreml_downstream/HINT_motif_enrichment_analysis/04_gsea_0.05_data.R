...
sig_regulons_hsc <- c(as.character(regulon_HSCa$X[which(regulon_HSCa$pval_wilcox <= 0.05)]), as.character(regulon_HSCb$X[which(regulon_HSCb$pval_wilcox <= 0.05)]))
...
for(i in 1:length(clusters)){
  ...
  write.csv(plot_data_final, file=paste0(OUT_DIR,"scatterplot_data/0.05_scatterplot_data_",clusters[i],".csv"))
