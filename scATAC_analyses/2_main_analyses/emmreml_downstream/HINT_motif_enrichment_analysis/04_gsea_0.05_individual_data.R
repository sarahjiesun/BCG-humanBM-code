...
for(i in 1:length(clusters)){
  ...
  sig_regulons <- c(regulon_HSCa$X[which(round(regulon_HSCa$pval_wilcox,2) <= 0.05)], regulon_HSCb$X[which(round(regulon_HSCb$pval_wilcox,2) <= 0.05)])
  ...
  write.csv(df_scatter, file=paste0(OUT_DIR,"scatterplot_data_individual/0.05_scatterplot_data_",clusters[i],".csv"))
